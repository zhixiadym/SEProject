...
% 2. 增加mu属性
% 3.修改某些property的计算方式
% 4.先检验t是否显著，然后再用剔除t的beta和alpha检验平稳性
% 5.提升速度,回归将fitlm改为regstats,集中赋值zeros(11,1)
% 6.放松对趋势项t的限制，即windowTest/3改为windowTest/2，超过半数才判断为不成立

classdef PairTradingSignalV5 < handle
    ...
    methods (Access = public) 
        function obj = PairTradingSignalV5(startDate, endDate, windowSmooth, windowReg,stock_sector, windowTest)
        ...
        function [Y,X] = cointegration(obj,dateLoc,stock1,stock2)
            ...
            if YNaNNum+XNaNNum >= 1 || max(Y_stat(:,3)) > 20|| max(X_stat(:,3)) > 20
                ...
                regResults = regstats(Y,[t,X],'linear',{'tstat','beta'});
                pValueAlpha2 = regResults.tstat.pval(2);
                %参考链接：https://ww2.mathworks.cn/help/releases/R2021a/stats/fitlm.html
                %mdl = fitlm([t, X],Y);
                %result = mdl.Coefficients.Variables;

                %obj.regAlpha1History(stock1,stock2,dateLoc) = result(1,1);
                obj.regAlpha1History(stock1,stock2,dateLoc) =  regResults.beta(1);
                if pValueAlpha2 < 0.01 %说明时间这个参数是显著的
                    obj.regAlpha2History(stock1,stock2,dateLoc) = 1;
                else
                    obj.regAlpha2History(stock1,stock2,dateLoc) = 0;
                end
                obj.regBetaHistoryT(stock1,stock2,dateLoc) = regResults.beta(3);
            ...
            for stock1 = 1:1:obj.stockNum-1
                for stock2 = stock1+1:1:obj.stockNum
                    ...
                    if alphaNaNNum+betaNaNNum == 0         
                        ...
                        %时间的beta 1/3以上都显著，协整关系直接不成立
                        if sum(alpha2Series) <= obj.windowTest/2
